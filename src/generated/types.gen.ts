...
export type AssistantMessage = Omit<ChatCompletionMessage, 'role'> & {
  ...
  /**
   * Model reasoning separated from the answer by the host's reasoning parser (vLLM convention).
   */
...
export type BlobTrainingDataItem = {
   ...
   * a /v2/consumer/blobs/{blobKey} URL, or a bare blob key; normalized to the AIR urn form at submission.
   */
  air: string;
  /**
   * Optional caption for this item, written as the trainer's sidecar caption file.
   */
  caption?: null | string;
};

/**
 * Training data supplied as individually uploaded blobs (via POST /v2/consumer/blobs),
 * so each item passed through per-asset moderation on upload.
 */
export type BlobsTrainingData = Omit<TrainingData, 'type'> & {
  /**
   * The training items, in dataset order. Each references a consumer-uploaded blob.
   */
  items: Array<BlobTrainingDataItem>;
  type: 'blobs';
};
...
export type ComfyBooguBaseCreateImageGenInput = Omit<
  ComfyBooguBaseImageGenInput,
...
export type ComfyBooguBaseImageGenInput = Omit<
...
export type ComfyBooguEditImageGenInput = Omit<
  ComfyBooguImageGenInput,
  'engine' | 'ecosystem' | 'model'
> & {
  operation: string;
...
export type ComfyBooguEditImageInput = Omit<
  ComfyBooguEditImageGenInput,
  'engine' | 'ecosystem' | 'model' | 'operation'
> & {
  width?: number;
  height?: number;
...
export type ComfyBooguTurboCreateImageGenInput = Omit<
  ComfyBooguTurboImageGenInput,
  'engine' | 'ecosystem' | 'model' | 'operation'
> & {
  width?: number;
  height?: number;
...
export type ComfyBooguTurboImageGenInput = Omit<
  ComfyBooguImageGenInput,
  'engine' | 'ecosystem' | 'model'
> & {
  operation: string;
...
export type ComfyNodepackSnapshotInput = {
  ...
  /**
   * Optional Sage Attention toggle, forwarded to the worker's launch so the
   * captured environment matches how the consumer runs the image.
   */
...
export type ComfyNodepackSnapshotResult = {
  /**
   * The pack AIR this result is for.
   */
  nodepack: string;
  /**
...
export type ComfyNodepackSnapshotStep = Omit<WorkflowStep, '$type'> & {
  input: ComfyNodepackSnapshotInput;
  output?: ComfyNodepackSnapshotOutput;
  $type: 'comfyNodepackSnapshot';
};

export type ComfyNodepackSnapshotStepTemplate = Omit<WorkflowStepTemplate, '$type'> & {
  input: ComfyNodepackSnapshotInput;
  $type: 'comfyNodepackSnapshot';
};
...
export type CustomComfyUsage = {
  buzzPerSecond: number;
  runtimeSeconds: number;
  /**
   * Estimate as of Civitai.Orchestration.Grains.Workflows.Steps.CustomComfy.CustomComfyUsage.ComputedAt: `Max(1, RuntimeSeconds × BuzzPerSecond)` (mirrors the billing floor).
   */
  estimatedCost: number;
  /**
   * When GPU runtime began accruing, or null before the worker reports. Distinct
   * from `WorkflowStep.StartedAt`, which is the earlier (unbilled) claim time.
   */
 ...
 * Animate a single source image (used as the first frame) into a video.
 */
export type HappyHorseV11ImageToVideoInput = Omit<
  HappyHorseV11VideoGenInput,
  'engine' | 'version' | 'operation'
> & {
  /**
   * Either A URL, A DataURL or a Base64 string
 ...
 * Reference subjects via "character1" through "character9" placeholders in the prompt.
 */
export type HappyHorseV11ReferenceToVideoInput = Omit<
  HappyHorseV11VideoGenInput,
  'engine' | 'version' | 'operation'
> & {
  images: Array<string>;
  aspectRatio?: '16:9' | '9:16' | '1:1' | '4:3' | '3:4' | '21:9' | '9:21' | '5:4' | '4:5';
  operation: 'referenceToVideo';
  version: 'v1.1';
  engine: 'happyHorse';
};
 ...
 * Generate a video from a text prompt only.
 */
export type HappyHorseV11TextToVideoInput = Omit<
  HappyHorseV11VideoGenInput,
  'engine' | 'version' | 'operation'
> & {
  aspectRatio?: '16:9' | '9:16' | '1:1' | '4:3' | '3:4' | '21:9' | '9:21' | '5:4' | '4:5';
...
/**
 * Represents joint face-and-body apparent-age results for photo and anime content.
 */
export type JointAgeClassificationResult = {
  detections: Array<JointAgeDetection>;
  status?: null | string;
  ran?: null | boolean;
  childDetected?: null | boolean;
  minorDetected?: null | boolean;
...
/**
 * Represents one photo-or-anime age estimate for a detected person.
 */
export type JointAgeDetection = {
  boundingBox: BoundingBox;
  personDetected: boolean;
  personDetectionConfidence: number;
  faceBoundingBox?: BoundingBox;
...
/**
 * Text-to-image, or image-to-image when Civitai.Orchestration.Grains.Workflows.Steps.ImageGen.Krea.KreaCreateImageGenInput.Image is supplied.
 */
export type KreaCreateImageGenInput = Omit<KreaImageGenInput, 'engine' | 'operation'> & {
  /**
   * Either A URL, A DataURL or a Base64 string
   */
  image?: null | string;
  /**
   * Denoising strength for image-to-image; ignored without Civitai.Orchestration.Grains.Workflows.Steps.ImageGen.Krea.KreaCreateImageGenInput.Image.
   */
  strength?: number;
  operation: 'createImage';
  engine: 'krea';
...
/**
 * Krea 2 served by Krea's own API. Distinct from `fal`/`krea2` (the same models via FAL) and
 * from the `comfy`/`krea2` ecosystem, which runs the open weights on our own workers.
 */
export type KreaImageGenInput = Omit<ImageGenInput, 'engine'> & {
  operation: null | string;
  prompt: string;
  model?: 'krea2-medium' | 'krea2-large' | 'krea2-medium-turbo';
  aspectRatio?: '1:1' | '4:3' | '3:2' | '16:9' | '2.35:1' | '4:5' | '2:3' | '9:16';
  /**
   * How much Krea expands the prompt: `raw` renders only what was written.
   */
  creativity?: 'raw' | 'low' | 'medium' | 'high';
  quantity?: number;
  seed?: null | number;
...
export type QwenImageBenchInput = {
  ...
  seed?: number;
  /**
   * Top-level benchmark dimensions to evaluate. Leave empty to evaluate all dimensions.
   */
  dimensions: Array<string>;
};

/**
 * A single generated image and the prompt it should be judged against.
 */
export type QwenImageBenchItem = {
  /**
...
export type QwenImageBenchOutput = {
   ...
   */
  results: {
    [key: string]: QwenImageBenchItemResult;
  };
};
 ...
 * Qwen Image Bench
 */
export type QwenImageBenchStep = Omit<WorkflowStep, '$type'> & {
  input: QwenImageBenchInput;
  output?: QwenImageBenchOutput;
  $type: 'qwenImageBench';
};
 ...
 * Qwen Image Bench
 */
export type QwenImageBenchStepTemplate = Omit<WorkflowStepTemplate, '$type'> & {
  input: QwenImageBenchInput;
  $type: 'qwenImageBench';
};
...
/**
 * How much of a custom-comfy job's ComfyUI `/ws` activity to record to a
 * streaming blob the consumer can tail live.
 */
...
export type Trellis2ComfyPolyGenInput = Omit<ComfyPolyGenInput, 'engine' | 'model'> & {
  operation: null | string;
  modelVersion?: 'pixal3D' | 'trellis2';
  modelAir?: null | string;
  structureSteps?: number;
  shapeSteps?: number;
  upsampleSteps?: number;
  textureSteps?: number;
  structureCfg?: number;
  shapeCfg?: number;
  textureCfg?: number;
  targetResolution?: 1024 | 1152 | 1280 | 1408 | 1536 | 1664 | 1792 | 1920 | 2048;
  shouldRemesh?: boolean;
...
export type Trellis2ImageTo3dComfyPolyGenInput = Omit<
  Trellis2ComfyPolyGenInput,
  'engine' | 'model' | 'operation'
> & {
  imageUrl: string;
  shouldTexture?: boolean;
...
export type Trellis2ShapeGenComfyPolyGenInput = Omit<
  Trellis2ComfyPolyGenInput,
  'engine' | 'model' | 'operation'
> & {
  imageUrl: string;
...
export type Trellis2TexGenComfyPolyGenInput = Omit<
  Trellis2ComfyPolyGenInput,
  'engine' | 'model' | 'operation'
> & {
  imageUrl: string;
...
export type Wan30ImageToVideoInput = Omit<
  Wan30VideoGenInput,
  'engine' | 'version' | 'operation'
> & {
  /**
  ...
  startImage?: null | string;
  /**
   * Constrains the last frame, for loops and transitions.
   */
  endImage?: null | string;
  operation: 'image-to-video';
  version: 'v3.0';
...
export type Wan30TextToVideoInput = Omit<Wan30VideoGenInput, 'engine' | 'version' | 'operation'> & {
  aspectRatio?: '16:9' | '9:16' | '1:1' | '4:3' | '3:4';
  operation: 'text-to-video';
  version: 'v3.0';
  engine: 'wan';
...
export type Wan30VideoGenInput = Omit<WanVideoGenInput, 'engine' | 'version'> & {
  operation: null | string;
  resolution?: '480p' | '720p' | '1080p';
  negativePrompt?: null | string;
  /**
   * Prompt rewriting. Disabling saves 20-60s of latency but may reduce generation quality —
   * follow Alibaba's prompting guide when turning it off.
   */
  enablePromptExpansion?: boolean;
  /**
   * Run on wan3.0-video-prime: same output quality, lower latency, higher price.
   */
  usePrime?: boolean;
  version: 'v3.0';
  engine: 'wan';
...
/**
 * Which generation-prompt field(s) the model is asked to evaluate in Prompt mode.
 */
export const XGuardPromptScope = {
  COMBINED: 'combined',
  POSITIVE: 'positive',
  NEGATIVE: 'negative',
} as const;
 ...
 * Which generation-prompt field(s) the model is asked to evaluate in Prompt mode.
 */
export type XGuardPromptScope = (typeof XGuardPromptScope)[keyof typeof XGuardPromptScope];
 ...
 * An assistant message representing a prior response.
 */
export type AssistantMessageWritable = Omit<ChatCompletionMessageWritable, 'role'> & {
  /**
   * The assistant message content (text only).
   */
  content?: null | string;
  ...
  reasoning_content?: null | string;
  /**
   * Input alias for the vLLM 0.26+ / OpenRouter field name; responses always use reasoning_content.
   */
 ...
 * ChatCompletion
 */
export type ChatCompletionStepWritable = Omit<WorkflowStepWritable, '$type'> & {
  input: ChatCompletionInput;
  output?: ChatCompletionOutputWritable;
  $type: 'chatCompletion';
};
...
export type InvalidateUserTokensData = {
  body?: never;
  path: {
    userId: number;
  };
  query?: never;
  url: '/v2/consumer/civitai/{userId}/tokens';
};

export type InvalidateUserTokensErrors = {
  /**
   * Unauthorized
   */
  401: ProblemDetails;
...
export type InvalidateUserTokensError =
  InvalidateUserTokensErrors[keyof InvalidateUserTokensErrors];

export type InvalidateUserTokensResponses = {
  /**
   * No Content
   */
  204: void;
...
export type InvalidateUserTokensResponse =
  InvalidateUserTokensResponses[keyof InvalidateUserTokensResponses];
...
export type InvokeComfyNodepackSnapshotStepTemplateData = {
  body?: ComfyNodepackSnapshotInput;
  path?: never;
  query?: {
    experimental?: boolean;
    allowMatureContent?: boolean;
...
export type InvokeComfyNodepackSnapshotStepTemplateErrors = {
  /**
   * Bad Request
   */
  400: ProblemDetails;
...
export type InvokeComfyNodepackSnapshotStepTemplateError =
  InvokeComfyNodepackSnapshotStepTemplateErrors[keyof InvokeComfyNodepackSnapshotStepTemplateErrors];

export type InvokeComfyNodepackSnapshotStepTemplateResponses = {
  /**
   * OK
   */
  200: ComfyNodepackSnapshotOutput;
};

export type InvokeComfyNodepackSnapshotStepTemplateResponse =
  InvokeComfyNodepackSnapshotStepTemplateResponses[keyof InvokeComfyNodepackSnapshotStepTemplateResponses];
...
export type InvokeQwenImageBenchStepTemplateErrors = {
  /**
   * Bad Request
   */
  400: ProblemDetails;
...
export type InvokeQwenImageBenchStepTemplateError =
  InvokeQwenImageBenchStepTemplateErrors[keyof InvokeQwenImageBenchStepTemplateErrors];

export type InvokeQwenImageBenchStepTemplateResponses = {
  /**
   * OK
   */
  200: QwenImageBenchOutput;
};

export type InvokeQwenImageBenchStepTemplateResponse =
  InvokeQwenImageBenchStepTemplateResponses[keyof InvokeQwenImageBenchStepTemplateResponses];
