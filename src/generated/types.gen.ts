...
export type CustomComfyOutput = {
  ...
  /**
   * Transient preview blobs — ComfyUI `type=temp` assets, e.g. a
   * `PreviewImage` node. Kept separate from Civitai.Orchestration.Grains.Workflows.Steps.CustomComfy.CustomComfyOutput.Blobs so
   * deliverable consumers keep seeing only real outputs; a hosted editor
   * resolves previews from here. Capped per job by the worker.
   */
